from app.modules.posts.service_layer.use_cases.update import UpdatePost, UpdatePostRequest, UpdatePostResponse
...
def test_update_post():
    ...
    post = posts[0]
    req = UpdatePostRequest(id=post.id, title="modified title", content=post.content, requested_user_id=post.user_id)

    # when
    res = service.execute(req)

    # then
    assert res == UpdatePostResponse(id=post.id, title="modified title", content=post.content, user_id=post.user_id)
